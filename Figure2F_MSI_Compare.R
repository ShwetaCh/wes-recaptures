...
msi1 = ggplot(comb_msi, 
  ...
  geom_abline(intercept = 0, slope = 1, col = "black") + 
  annotate('text', y = c(10)+0.5, x = 40, color = 'darkred', label = c('MSI high (10)'), size = 3.5) +
  annotate('text', x = c(10)-0.5, y = 40, color = 'darkred', label = c('MSI high (10)'), size = 3.5, angle = 90) +
  # theme(axis.text.x = element_text(colour = "blue")) +
  # theme(axis.text.y = element_text(colour = "blue")) +
  theme(plot.title = element_text(size = 14),plot.margin = unit(c(1,1,1,1), 'lines')) +
  labs(title = paste0(#"MSI-sensor score","\n",
...
print("Zoomed in scatter plot--->") 
######--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
comb_msi = comb_msi %>% mutate(.,MSI_Status=ifelse(MSI_Exome>=10 & MSI_IMPACT>=10,"Concordant","Discordant"));
comb_msi = comb_msi %>% mutate(.,MSI_Status=ifelse(MSI_Exome<3 & MSI_IMPACT<3,"Concordant",MSI_Status));

#comb_msi = comb_msi %>% mutate(.,MSI_Status=ifelse(MSI_Exome<3.5 & MSI_IMPACT<10,"Discordant MSI_Status","Concordant MSS"));

#comb_msi = comb_msi %>% mutate(.,MSI_Status=ifelse(MSI_Exome<10 & MSI_IMPACT>=3,"MSI-I",MSI_Status));
comb_msi = comb_msi %>% mutate(.,MSI_Status=ifelse(MSI_IMPACT<10 & MSI_IMPACT>=3,"MSI-I",MSI_Status));
#(MSI_Exome>=10 & MSI_Exome<10) & 
#pdf(file='zoomed_msi_comparison_022719.pdf', width = 11, height = 8)
...
msi2 = ggplot(comb_msi,aes(y=MSI_Exome,x=MSI_IMPACT,color=MSI_Status) )+
  ...
  #geom_abline(col = "black",slope=1, intercept =0) + 
  #geom_hline(yintercept = 3.5, linetype = 'dashed', color = 'red') +
  geom_abline(intercept = 0, slope = 1) +  geom_point(pch = 21, size = 2, fill = 'lightgray') + ylim(c(0, 50)) + xlim(c(0,50)) + 
  scale_colour_manual(values = c("forestgreen","red","#08519c","orange")) +
  # theme(axis.text.x = element_text(colour = "blue")) +
  # theme(axis.text.y = element_text(colour = "blue")) +
  theme(plot.title = element_text(size = 14)) +
  annotate('text', y = c(10)+.25, x = 7.5, color = 'darkred', label = c('10'), size = 3.5) +
  annotate('text', x = c(10)-.25, y = 14, color = 'darkred', label = c('10'), size = 3.5) +
  annotate('text', x = c(3)-.25, y = 14, color = 'black', label = c('3'), size = 3.5) +
  theme(legend.position = 'bottom',legend.title=element_blank(),legend.justification = "left",legend.direction = "vertical")
msi2

pdf("~/tempo-cohort-level/Figure2F_MSI_Compare_v2.pdf", paper = "a4r")
grid.arrange(msi1, msi2, ncol = 2, newpage = F)
dev.off()
